...
struct net_device_ops {
    ...
    /*注册网络设备时调用， 驱动程序一般不使用，虚拟网络设备可能会用到*/
	int			(*ndo_init)(struct net_device *dev);
    /*网络设备卸载时调用， 驱动程序一般都会用到*/
	void			(*ndo_uninit)(struct net_device *dev);
    /*
     * 1.禁止所有中断、关闭载波（netif_carrier_off）;
     * 2.分配并初始化传输和接受描述符环内存（使用dma_alloc-coherent分配确保EMAC和CPU都可以访问）;
     * 3.分配发送skb处理状态的链表或者数组空间（使用kzalloc），其实也可以用sk_buff_head相关的数据结构和函数;
     * 4.初始化接收描述符环，包括分配struct sk_buff以及数据缓存（netdev_alloc_skb），并且进行DMA映射（dma_map_single）;
     ...
     * 10.调用netif_wake_queue或者netif_tx_wake_all_queues开启传输队列的调度（因为此时所有的传输环为空，可以i立刻开始传输）;
	...
	int			(*ndo_stop)(struct net_device *dev);
    /*
     * 1.检查skb_shinfo(skb)->tx_flags标志位,SKBTX_HW_TSTAMP指示硬件向上层软件提供硬件时间戳（调用skb_tstamp_tx、skb_tx_timestamp），
     ...
     * 4.queue_mapping指示SKB需要插入到哪个传输队列上;
     ...
     * 11.检查skb是否为jumbo帧，硬件师傅哦支持直接发送
     * 12.检查skb->no_fcs确定是否听见需要在帧尾部加上FCS
     */
	netdev_tx_t		(*ndo_start_xmit)(struct sk_buff *skb,
						  struct net_device *dev);
	netdev_features_t	(*ndo_features_check)(struct sk_buff *skb,
	...
	int			(*ndo_change_mtu)(struct net_device *dev,
						  int new_mtu);
    /*设置邻居子系统struct neigh_parms参数，驱动不用管*/
	int			(*ndo_neigh_setup)(struct net_device *dev,
						   struct neigh_parms *);
    /*此时多半MAC已经宕机了，当然实际处理应该因人而异，不一定非得进行一次完全复位初始化操作
     * 1.保存现场快照
     * 2.关闭MAC和PHY
     * 3.复位并重新初始化MAC和PHY
     */
    ...
    /*若网卡支持VLAN的VID过滤，则从过滤列表中删除对应的VID*/
	...
	int			(*ndo_fcoe_ddp_done)(struct net_device *dev,
						     u16 xid);
    /*执行DDP（Direct Data Placement Protocol）相关的初始操作，特别是struct scatterlist的DMA初始化*/
...
#define NETDEV_FCOE_WWPN 1
    /*返回驱动自定义的wwn值，传入的参数yupe可能是NETDEV_FCOE_WWNN或者NETDEV_FCOE_WWPN*/
	int			(*ndo_fcoe_get_wwn)(struct net_device *dev,
						    u64 *wwn, int type);
#endif

#ifdef CONFIG_RFS_ACCEL
    /*设置RFS硬件过滤器，关于RFS参考《Rceveive flow steering》*/
