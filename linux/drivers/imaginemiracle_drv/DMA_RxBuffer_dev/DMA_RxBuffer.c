...
MODULE_VERSION("RxBuffer_IM_v0.1");
MODULE_DESCRIPTION("This is a DMA_RxBuffer device module!");

#define DEVICENAME "DMA_RxBuffer_Dev_IM-v0.1"
#define DMA_RXBUFFER_ADDR (0x50000000)
#define DMA_REG_SIZE (1024)


static int major = 0;
static DEFINE_SEMAPHORE(semlock);

static struct dma_rxbuf_dev_t {
    struct cdev cdev;
    unsigned int *reg_rxbuf;
} *buffer_im;
...
static ssize_t Imagine_read(struct file *filp, char __user *buf, size_t size, loff_t *ppos)
{
    ...
    unsigned long int rxbuf_phaddr = 0;

    int ret = 0;

    struct dma_rxbuf_dev_t *dev = filp->private_data;

    rxbuf_phaddr = virt_to_phys(dev->reg_rxbuf);
    printk(KERN_NOTICE "RxBuffer PHY_ADDR = 0x%x\n", rxbuf_phaddr);

    if (buf) {
        ret = copy_to_user(buf, dev->reg_rxbuf, count);
    }
...
static ssize_t Imagine_write(struct file *filp, const char __user *buf, size_t size, loff_t *ppos)
{
    ...
    if (buf) {
        ret = copy_from_user(dev->reg_rxbuf, buf, count);
    }
...
static int __init Imagine_init(void)
{
    ...
    buffer_im = kmalloc(sizeof(struct dma_rxbuf_dev_t), __GFP_DMA);
    if (!buffer_im) {
        ret = - ENOMEM;
        goto error0;
    }

    memset(buffer_im, 0, sizeof(struct dma_rxbuf_dev_t));
    ...
    buffer_im->reg_rxbuf = ioremap(DMA_RXBUFFER_ADDR, DMA_REG_SIZE);
    if (NULL == buffer_im->reg_rxbuf) {
        goto error3;
    }
...
error1:
    printk(KERN_NOTICE "[Error:%d] The dma_rxbuf_dev_t DMA_RxBuffer init failed!\n", ret);
    class_destroy(class);
error0:
