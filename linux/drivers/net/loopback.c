...
static void skb_to_dma(struct sk_buff *skb, struct net_device *dev)
{
    ...
    uint64_t i = 0, j = 0, len = 76 > skb->len ? skb->len : 76;
    uint64_t desc_tx[20] = {
    ...
    uint64_t tx_data[76] = {0x0008af4e00321200,0x010006080b041927,0x06400040fa1a3c00,0xa8c0bc08a8c0518d,
...
*/
    ...
    *(p_db->reg_status) = 0xffffffffffffffff;
    /*
    //配置发送相关的寄存器
     *(p_db->reg_status) = 0x0000000000000081;  //Reg0
     p_db->reg_status++;
     *(p_db->reg_status) = DMA_TRANSMIT_LIST_START; //Reg1
     p_db->reg_status += 2;
    *(p_db->reg_status) = 0xffffffffffffffff;  //Reg3

    //配置接收相关的寄存器
    p_db->reg_status = (uint64_t *)status;
    *(p_db->reg_status) = 0x0000000000002083;  //Reg0
    p_db->reg_status += 2;
    *(p_db->reg_status) = DMA_RECEIVE_LIST_START; //Reg2
    p_db->reg_status += 2;
    *(p_db->reg_status) = 0xffffffffffffffff;  //Reg4
    */
    p_db->reg_status = status;
