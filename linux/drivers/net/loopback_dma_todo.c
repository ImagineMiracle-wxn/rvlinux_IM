...
static void skb_to_dma(struct sk_buff *skb, struct net_device *dev)
{
    ...
    uint64_t i = 0, j = 0;
    uint64_t pnum = DMA_PACKETS_NUM(skb->len),
             len = DMA_BUFFER_DBARR_SIZE(pnum);
    uint64_t *desc_tx = NULL, *desc_rx = NULL;

    uint64_t txbuf_d_naddr = 0x0000000040000000,
    ...
    desc_tx = (uint64_t *)kzalloc(len * sizeof(uint64_t), GFP_KERNEL);
    desc_rx = (uint64_t *)kzalloc(len * sizeof(uint64_t), GFP_KERNEL);

    printk(KERN_NOTICE "kzalloc ok!\n");

    for (j = 0, i = 0; j < pnum; j++) {
    ...
    printk(KERN_NOTICE "Set txbuf ok!\n");
    ...
    printk(KERN_NOTICE "Set rxbuf ok!\n");

    memcpy(p_db->io_t_base, skb->data, skb->len < DMA_BUFFER_SIZE ? skb->len : DMA_BUFFER_SIZE);

    for (i = 0; i < skb->len;) {
        printk(KERN_NOTICE "%016lx,%016lx,%016lx,%016lx,\n", skb->data[i], skb->data[i + 1], skb->data[i + 2], skb->data[i + 3]);
        i += 4;
    }

    printk(KERN_NOTICE "skb data to dma txBuffer[%d bytes]!\n", skb->len);
    printk(KERN_NOTICE "[data to txbuf:] 0x%016x[skb->data = 0x%16lx]\n", p_db->io_t_base[0], skb->data[0]);

/*
    for (i = 0; i < len; i++) {
        *(db->io_t_base) = skb->data[i];
        db->io_t_base++;
    }
*/

    *(p_db->reg_status) = 0x0000000000002883;
    p_db->reg_status++;
    ...
    *(p_db->reg_status) = 0xffffffffffffffff;
    ...
    if (desc_tx && desc_rx) {
        ...
        printk(KERN_NOTICE "desc_tx: \n");
        for(i = 0; i < len;) {
            //printk(KERN_NOTICE "0x%016lx, 0x%016lx, 0x%016lx, 0x%016lx\n", p_db->io_t_base[i], desc_tx[i + 1], desc_tx[i + 2], desc_tx[i + 3]);
            printk(KERN_NOTICE "0x%016lx, 0x%016lx, 0x%016lx, 0x%016lx\n", p_db->io_t_base[i], p_db->io_t_base[i + 1], p_db->io_t_base[i + 2], p_db->io_t_base[i + 3]);
            i += 4;
        }

        printk(KERN_NOTICE "desc_rx: \n");
        for(i = 0; i < len;) {
            //printk(KERN_NOTICE "0x%016lx, 0x%016lx, 0x%016lx, 0x%016lx\n", p_db->io_t_base[i], desc_tx[i + 1], desc_tx[i + 2], desc_tx[i + 3]);
            printk(KERN_NOTICE "0x%016lx, 0x%016lx, 0x%016lx, 0x%016lx\n", p_db->io_r_base[i], p_db->io_r_base[i + 1], p_db->io_r_base[i + 2], p_db->io_r_base[i + 3]);
            i += 4;
        }
    }
/*
...
*/
    printk(KERN_NOTICE "ioremap[%016lx]::ioremap[%016lx]\n", desc_tx[1], desc_rx[1]);
/*
    printk(KERN_NOTICE "SendData[0]: 0x%016lx\n", *((uint64_t *)ioremap(desc_tx[1], 1)));
    printk(KERN_NOTICE "RecvData[0]: 0x%016lx\n", *((uint64_t *)ioremap(desc_rx[1], 1)));
*/
    printk(KERN_NOTICE "SendData[0]: 0x%016lx\n", *((uint64_t *)ioremap(0x400000a0, 64)));
    printk(KERN_NOTICE "RecvData[0]: 0x%016lx\n", *((uint64_t *)ioremap(0x500000a0, 64)));
    return ;
}
